...
source("./tools/myfunctions.r")
theme_set(jalsggtheme)
```
...
![Geographical Location](./imgs/Ames_map.png){width=100%}

Ames, Iowa (USA)
:::
# Introduction

In this classic [Kaggle](https://www.kaggle.com/competitions/house-prices-advanced-regression-techniques) project you will see my workflow developing pipelines on the ETL phase of the DS cycle, as well as a tidy approach on the EDA substantialy supported on Visual Analytics.
...
df_all <- read_csv("./data/raw/train.csv") # All labeled data available
df_sub <- read_csv("./data/raw/test.csv") # Only features for prediction and sumbission
```

## Missing Values

My **first step** is always to check the completeness of data. Therefore, let's check for missing values[^1]. It is a recurrent first step in EDA. For this case I wrote the function *count_na()* that generates the following table:

[^1]: I encourage you to **write your own function to quantify missing values** depending on the programming language and labelling system used. 
...
Before jumping to any conclusion about the percentage of missing values, read thoroughly the data manifest *data_description.txt* available here:
...
There you'll see that for some columns, missing data (NA) means really *"None"*, i.e.,  the 
absence of the determined feature in the house is a category exposing the lack of such quality that can have a 
significant impact on the response variable Sale Price.
