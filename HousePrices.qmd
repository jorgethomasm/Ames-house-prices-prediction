...
1. One file contains everything, i.e., features and labels (predictors and response). This is the *training, validation and testing dataset*. I will have to split properly the *Data Budget*.
...
print("Data Budget")
dim(df_all)
print("Submission dataset of features")
dim(df_sub) 
```

In order to organise my **Data Budget** I count on 1460 Observations of 80 predictors and the Response, which is the Sale Price of the houses. 
...
Therefore, I will fill the empty fields (NA) of the indicated columns of both datasets with the string *"None"*.
...
```{r}
#| label: Submission Features
#| column: margin
#| tbl-cap: Submission Data
df_sub |> 
  count_na() |>
  knitr::kable()
```
...
2.  To provide simple syntax to write elements of the Tufte style such as side notes and margin figures. If you'd like a figure placed in the margin, just set the option `fig-column: margin` for your code chunk, and we will take care of the details for you.
