...
- **10-folds Cross-Validation?** Analysis (training folds), Assessment (validation folds) and test: it's *small data*, I can choose something even more powerful.
- **Bootstrap Sampling?** YES.

By today's standards this dataset is small, therefore I will choose Bootstraping (CV on steroids!) to keep the assessment set the same size as the original training set. Although performance metrics will be slightly conservative or pesimistic, I prefer it.
...
model_lm <-  linear_reg() |>
  set_engine("lm") |>
  set_mode("regression") |>
  fit(SalePrice ~ GrLivArea, data = ames_train)
...
# https://www.youtube.com/watch?v=GtAX7Y2F4aY
