...
set.seed(1982)
ames_boots <- bootstraps(ames_train, times = 200, strata = SalePrice)
ames_boots  
```
...
ames_train |>  
  select(c(GrLivArea, TotBaths, GarageCars, TotalBsmtSF, YearBuilt)) |> 
...
# Multi-lm 
multi_lm_rec <-
  recipe(SalePrice ~ GrLivArea + TotBaths + GarageCars + TotalBsmtSF + YearBuilt + OverallQual + KitchenQual + ExistFireplace, data = ames_train) |>
  step_BoxCox(SalePrice, limits = c(-1, 1)) |> 
...
My reference is the **linear GAM with Splines**, showing a mean RMSE = 0.0761. The result was transformed in the pipeline, therefore no units are displayed.

[I got slighlty better results using one-hot encoded categorical Feats. Moreover, applying the Box-Cox transformation to numeric predictors helped too! When they are skewed, the fit is dramatically affected. Remember @fig-cont_linear.]{.column-margin}
