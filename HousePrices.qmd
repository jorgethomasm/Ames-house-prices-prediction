...
subtitle: "A commented and visual walk-through advanced regression techniques." # only for html output
...
[After my second Kaggle submission (Top 25%) I completed the [Feature Engineering course](https://www.kaggle.com/learn/feature-engineering) that has a lot of tips! I included some new features and tricks from there.]{.column-margin}
...
`MSSubClass` has too many categories, therefore I will perform **target enconding** in the *recipe* as a step.
...
[I will use 10-folds CV reapeated 10 times (100 splits) for the hyperparameter tuning of more complex algoritms. Once optimised, I'll assess the test error again with the 1000 bootstraps, for the sake of comparison.]{.column-margin}
...
The following code shows the preprocessing steps enclosed in a pipeline or ***recipe***. The list of available *step functions* can be found [here](https://recipes.tidymodels.org/reference/index.html).
...
ames_train |>  
  select(c(GrLivArea, TotBaths, GarageArea, TotalBsmtSF, YearBuilt, SecondFlrSF)) |> 
...
[I got slightly better results using dummy encoded factors. Moreover, applying the Yeo-Johnson transformation to numeric Feats. helped too! They were skewed, see @fig-skewed-feats.]{.column-margin}

## Elastic-Net Regression

Having high multicollinearity it's reasonable to think of some sort of penalisation or shrinkage method. Hence, I'll go with a Generalised Linear Model, [GLM](https://glmnet.stanford.edu/articles/glmnet.html). In this manner I can have a first look at feature importance.
...
penalty_ <- 10^seq(-3, -2, length.out = 30)
...
A very low penalty of $\lambda = 0.0085$ with a mixture of $\alpha = 0.15$ were the best parameters.

New fit with new feature engineering, mean RMSE = 0.0637 (1000 Bootstraps). **Kaggle's Top 15%**.
