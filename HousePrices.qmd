...
subtitle: "My commented and visual walk-through advance regression techniques." # only for html output
...
Although **nothing is linear**, is good to have an idea of simple correlations between all variables. After going through the previous steps, it's obvious that there are correlations. Think about the number of cars that fit in a garage `GarageCars` and the area of the garage `GarageArea`.

```{r}
#| label: fig-num-corr
#| fig-width: 8
#| fig-height: 8
#| fig-cap: Pearson's correlation of numeric features and response.
#| cap-location: margin
#| warning: false
#| message: false
...
I'm leaving blank the insignificant correlations for better visualisation and focus. Now I'm checking the categorical variables and the response.
...
Basically all that has to do with *quality* ratings is very important for the final sale price.

### Analysis of the response variable: Sale Price

How well-behaved is the target? Let's see it:
...
The logarithmic transformation works when there are few large values, like in this case, there are only a few expensive hauses. The logarithm scale will shrink that right tail[^6], making the distribution of the values more normal-like.

[^6]: Tip: think of a **Logarithmic transformation** like pressing an **accordeon** only from your right arm.

However, because we have a computer and libraries with written functions to perform variable transformations, I will use the Box-Cox transformation, which is a family of transformations (functions) that includes the logarithmic one.
...
The new `SalePrice_bc` should be better behaved...
...
... and it is, indeed. Well, except for the tails now it's normal at the core.
...
One of the motivations to develop this project is because I'm switching to the **Tidymodels** package. Therefore it's time to load the library to train a simple reference model.
