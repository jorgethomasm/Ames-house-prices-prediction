...
ames_all <- ames_all |>
  ...
  # a dummy of PoolQC and PoolArea will makes this Feat. redundant!
...
## Choosing Resampling Technique
Depending on the chosen resample technique to estimate the Goodness-of-Fit (test error)
Take a look at the pristine explanation by [@resamplingML].
...
I will generate 300 bootstrap samples. 
...
set.seed(1982)
ames_boots <- bootstraps(ames_train, times = 300, strata = SalePrice)
ames_boots  
```
...
However, for a first model I'd like to keep interpretability  and to involve all predictors in order to apply some regularisation given the problem of multicollinearity. 
Hence, I'll go with a regularised (shrinkage) Generalised Linear Model, [GLM](https://glmnet.stanford.edu/articles/glmnet.html). In this manner I can have a first look at feature importance.

I'll choose Elastic net regression. Features must be scaled![^7]

[^7]: I don't want to over-penalised non-scaled variables using shrinkage methods.
...
num_mul_lm_recipe <-
  recipe(SalePrice ~ GrLivArea + GarageCars, data = ames_train) 
  #step_BoxCox(SalePrice, limits = c(-1, 1)) |> 
  #step_scale(all_numeric_predictors())
...
elnet_tune_res <- tune_grid(elnet_spec, 
                            preprocessor = shrinkage_spline_recipe, 
                            grid = elnet_grid, 
...
list_of_recipes <- list(num_uni_lm_recipe, num_mul_lm_recipe)
list_of_models <- list(lm_spec)
