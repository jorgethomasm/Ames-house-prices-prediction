...
Note that `Id` and the just added `dataset` columns are not predictors. Hence, there are **78 features** to play  with. Also rememeber that the number of features for modelling will vary. Some will be discarded and some will be created along the analysis.
...
1. Aggregation to a total of **continuous-numerical** surface area (Square Feet), e.g. new `TotInteriorArea` Feat. summing basement and floors, etc. Same for a new `TotExteriorArea`. Then deleting the addends variables.
...
One can hear often that *location is everything*; let's see here:
...
Using the **Mutual Information (MI)** criterion I'll rank the extent to which knowledge of one predictor reduces the uncertainty of `SalePrice`.
...
ames_all |>
  filter(dataset == "train") |>
  select(-c(Id, dataset)) |>
  calc_mi(target = "SalePrice") |>
  head(15) |>
  knitr::kable(caption = "Feat. Importance Screening")
...
ames_wfset <- workflow_set(my_lm_recipes, my_lms_specs, cross = TRUE, case_weights = NULL)
ames_wfset
...
ames_lms_res |>  
  ...
  filter(.metric == "rmse") |>
  knitr::kable(caption = "3 Base lms")
...
Ok! I got on the **top 59% (2825/4819)** cherry picking predictors and a GAM of splines. Not great, as expected, but it's an important referece.
...
# .csv for submission
sub_2_elasticnet <- data.frame(Id = ames_test$Id, SalePrice = res_2_elasticnet$SalePrice_pred)
write.csv(sub_2_elasticnet, "./data/submissions/sub_2_elasticnet.csv", quote = FALSE, row.names = FALSE)
