...
This is because the project is actually an open competition to see who makes the best predictions. That means for this case getting minimum RMSE.
...
One of the early and recurrent steps of the EDA is to check the **completeness of data**. Let's search for missing values, after filling indicated fields[^2]. For this case I wrote the function *count_na()* that generates the following tables:
...
[^3]: Tip: integers as Years can be treated as categories for the analysis.

```{r}
#| label: Feat. Engineering
...
Logarithmic transformations work when there are few large values, like in this case, there are only a few of expensive hauses. The logarithm scale will shrink that right tail.
