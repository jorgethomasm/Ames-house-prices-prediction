...
subtitle: "Advanced Regression and Visual Analytics Techniques" # only for html output
...
style is known for its extensive use of sidenotes, tight integration of graphics with text, and well-set typography. Quarto[^1] supports most of the layout techniques that are used in the Tufte handout style for both HTML and LaTeX/PDF output.
...
First things first, let's check for missing values:

```{r}
#| label: Missing Values

df_all |> count_na() |> View()
#sapply(df_all$RoofMatl, function(x) sum(is.na(x)))

df_sub |> count_na() |> View()

test <- df_all |> sapply(FUN = function(x) 100*sum(is.na(x))/nrow(df_all)) |> round(2) |> as_tibble() 
test2<- sapply(df_all, function(x) sum(is.na(x)))
# typeof(test)

as_tibble(data=test2, rownames_to_column())
